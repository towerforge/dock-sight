...
import { apiServiceInfo } from '@/services/sysinfo';
  ...
  return (
    <div className="flex flex-col gap-4">
      {data.containers.map((c: any) => (
        <div key={c.id} className="rounded-2xl bg-card-bg border border-card-border p-4 flex flex-col gap-3">

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${c.running ? 'bg-emerald-500' : 'bg-red-500'}`} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
            ...
            <Row icon={<Info size={12} />}      label="Command" value={c.command || '—'} mono />
